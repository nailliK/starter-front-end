...
class TodosContainer extends React.Component {
    ...
    render() {
        ...
        return (
            <main>
                <form>
                    <input type="text" placeholder="Add a new todo" value={this.state.newTodo}
                           onChange={this.onChangeNewTodo.bind(this)}/>
                    <button type="submit" onClick={this.addNewTodo.bind(this)} disabled={!this.state.newTodo}>Add
                    </button>
                </form>
                <ol>
                    {this.props.todos.map(function (todo) {
                        return <Todo key={`todo__${todo.id}`} todo={todo}
